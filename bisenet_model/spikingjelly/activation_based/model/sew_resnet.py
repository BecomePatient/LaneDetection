...
class SEWResNet(nn.Module):
    def __init__(self, block, layers, num_classes=1000, zero_init_residual=False,
        ...
        self.sn1 = spiking_neuron(**deepcopy(kwargs))
        self.maxpool = layer.MaxPool2d(kernel_size=3, stride=1, padding=1)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False, cnf: str=None, spiking_neuron: callable = None, **kwargs):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:
            ...
            downsample = norm_layer(self.inplanes)
    ...
    def _forward_impl(self, x):
        ...
        # print(f"输入图片张量大小为{x.shape}")
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.sn1(x)
        x = self.maxpool(x)
        # print(f"经过最大值池化张量大小为{x.shape}")

        x = self.layer1(x)
        # print("第一层layer")
        # print(self.layer1)
        # print(f"张量大小为{x.shape}")
        x = self.layer2(x)
        # print("第二层layer")
        # print(self.layer2)
        # print(f"张量大小为{x.shape}")
