...
class Converter(nn.Module):
    ...
    def forward(self, ann: nn.Module):
        ...
        * :ref:`API in Chinese <Converter.forward-cn>`
        ...
        ann = fx.symbolic_trace(ann, concrete_args={'augment': False,'profile':False, 'visualize':False,"val":False}).to(self.device)
        # print(type(ann))
        # print(ann)
        ann.eval()
        ...
        for _, (imgs, _, _, _) in enumerate(tqdm(self.dataloader)):
            imgs = imgs.to(self.device, dtype=torch.float32)  # 将输入数据转移到模型所在的设备，并使用 float32 数据类型
            # imgs /= 255
            ann_with_hook(imgs)
