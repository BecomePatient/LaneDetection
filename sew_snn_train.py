...
def train(args):
    ...
    for epoch in range(epoch,epochs):
        ...
        df.to_excel('snn_sew_loss_values.xlsx',index = False)

        # 保存最佳权重
        if avg_loss < best_loss:
            best_loss = avg_loss
            save_checkpoint(model, optimizer, epoch, best_loss, './weight/snn_sew_best_model_checkpoint.pth')
            # best_weights = model.state_dict()

        # 保存最后一轮权重
        if epoch == epochs - 1:
            save_checkpoint(model, optimizer, epoch, avg_loss, './weight/snn_sew_last_model_checkpoint.pth')
