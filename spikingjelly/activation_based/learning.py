...
def stdp_maxpool_single_step(
    max_pool: nn.MaxPool2d, in_spike: torch.Tensor, out_spike: torch.Tensor,
    trace_pre: Union[torch.Tensor, None], trace_post: Union[torch.Tensor, None],
    tau_pre: float, tau_post: float,
    f_pre: Callable = lambda x: x, f_post: Callable = lambda x: x
):
    if max_pool.dilation != (1, 1):
        raise NotImplementedError(
            'STDP with dilation != 1 for MaxPool2d has not been implemented!'
        )
    ...
    delta_w = torch.zeros_like(in_spike)
    for b in range(in_spike.shape[0]):
        for c in range(in_spike.shape[1]):
            for h in range(in_spike.shape[2]):
                for w in range(in_spike.shape[3]):
                    h_start = h * max_pool.stride[0]
                    h_end = (h + 1) * max_pool.stride[0]
                    w_start = w * max_pool.stride[1]
                    w_end = (w + 1) * max_pool.stride[1]

                    # Find the maximum activation within the pooling region
                    max_activation = in_spike[b, c, h_start:h_end, w_start:w_end].max()

                    # Compute the pre and post traces
                    tr_pre = trace_pre[b, c, h, w]
                    tr_post = trace_post[b, c, h, w]

                    delta_w_pre = - (f_pre(max_activation) * tr_post)
                    delta_w_post = f_post(max_activation) * tr_pre

                    # Accumulate the weight updates
                    delta_w[b, c, h_start:h_end, w_start:w_end] += delta_w_pre + delta_w_post

    return trace_pre, trace_post, delta_w


def stdp_adaptive_avg_pool2d_single_step(
    pool: nn.AdaptiveAvgPool2d, in_spike: torch.Tensor, out_spike: torch.Tensor,
    trace_pre: Union[torch.Tensor, None], trace_post: Union[torch.Tensor, None],
    tau_pre: float, tau_post: float,
    f_pre: Callable = lambda x: x, f_post: Callable = lambda x: x
):
    if in_spike.ndim != 4 or out_spike.ndim != 4:
        raise ValueError(
            'Input and output spikes should have 4 dimensions (batch_size, channels, height, width)'
        )
    ...
    delta_w = torch.zeros_like(pool.weight.data)
    for h in range(pool.output_size[0]):
        for w in range(pool.output_size[1]):
            h_start = h * pool.stride[0]
            h_end = min(h_start + pool.kernel_size[0], in_spike.shape[2])
            w_start = w * pool.stride[1]
            w_end = min(w_start + pool.kernel_size[1], in_spike.shape[3])

            pre_spike = in_spike[:, :, h_start:h_end, w_start:w_end]
            post_spike = out_spike[:, :, h, w]
            weight = pool.weight.data[:, :, h, w]

            tr_pre = trace_pre[:, :, h_start:h_end, w_start:w_end]
...
def stdp_batchnorm2d_single_step(
    bn: nn.BatchNorm2d, in_spike: torch.Tensor, out_spike: torch.Tensor,
    trace_pre: Union[torch.Tensor, None], trace_post: Union[torch.Tensor, None],
    tau_pre: float, tau_post: float,
    f_pre: Callable = lambda x: x, f_post: Callable = lambda x: x
):
    ...
    delta_w = torch.zeros_like(bn.weight.data)

    if bn.affine:
        w = bn.weight.data
    else:
        w = torch.ones_like(bn.running_var.data)

    delta_w_pre = - (f_pre(w) *
                    trace_post.unsqueeze(2).unsqueeze(2) *
                    (in_spike.unsqueeze(1) - bn.running_mean.data.unsqueeze(0)) /
                    torch.sqrt(bn.running_var.data + bn.eps))
    delta_w_post = f_post(w) * (trace_pre.unsqueeze(1).unsqueeze(1) *
                                (out_spike.unsqueeze(2) - bn.running_mean.data.unsqueeze(0)) /
                                torch.sqrt(bn.running_var.data + bn.eps))
    delta_w += delta_w_pre.sum(dim=(0, 2, 3))
    delta_w += delta_w_post.sum(dim=(0, 2, 3))

    return trace_pre, trace_post, delta_w


def stdp_multi_step(
    layer: Union[nn.Linear, nn.Conv1d, nn.Conv2d,nn.MaxPool2d,nn.AdaptiveAvgPool2d],
    ...
    if isinstance(layer, nn.Linear):
        ...
        stdp_single_step = stdp_maxpool_single_step
    elif isinstance(layer, nn.BatchNorm2d):
        stdp_single_step = stdp_batchnorm2d_single_step
    elif isinstance(layer, nn.AdaptiveAvgPool2d):
        stdp_single_step = stdp_adaptive_avg_pool2d_single_step
...
class STDPLearner(base.MemoryModule):
    def __init__(
        self, step_mode: str,
        synapse: Union[nn.Conv2d, nn.Linear,nn.MaxPool2d,nn.AdaptiveAvgPool2d], sn: neuron.BaseNode,
        tau_pre: float, tau_post: float,
        f_pre: Callable = lambda x: x, f_post: Callable = lambda x: x
    ):
        super().__init__()
    ...
    def step(self, on_grad: bool = True, scale: float = 1.):
        ...
        if self.step_mode == 's':
            ...
            elif isinstance(self.synapse, nn.MaxPool2d):
                stdp_f = stdp_maxpool_single_step
            elif isinstance(self.synapse, nn.BatchNorm2d):
                stdp_f = stdp_batchnorm2d_single_step
            elif isinstance(self.synapse, nn.AdaptiveAvgPool2d):
                stdp_f = stdp_adaptive_avg_pool2d_single_step
            else:
                raise NotImplementedError(self.synapse)
        elif self.step_mode == 'm':
            if isinstance(self.synapse, (nn.Linear, nn.Conv1d, nn.Conv2d,nn.MaxPool2d,nn.AdaptiveAvgPool2d)):
        ...
        if on_grad:
            ...
            #print(f" self.synapse.weight.grad = { self.synapse.weight.grad}")
        else:
            return delta_w
