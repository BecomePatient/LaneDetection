...
if __name__ == "__main__":
    model = STDCNet813(num_classes=2, dropout=0.00, block_num=4)
    model.eval()
    x = torch.randn(1,3,224,224)
    y = model(x)
    # torch.save(model.state_dict(), 'cat.pth')
    # print(y.size())
    print(f"y0 = {y[0].shape}")
    print(f"y1 = {y[1].shape}")
    print(f"y2 = {y[2].shape}")
    print(f"y3 = {y[3].shape}")
    print(f"y4 = {y[4].shape}")
