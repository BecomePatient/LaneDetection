...
class _LaneNetCluster(object):
    ...
    def _embedding_feats_dbscan_cluster(self, embedding_image_feats):
        ...
        :return:
        """
        db = DBSCAN(eps=0.3, min_samples=1500,algorithm = 'ball_tree', leaf_size = 500)
        try:
            start_time = time.time()
            features = StandardScaler().fit_transform(embedding_image_feats)
            end_time = time.time()
            elapsed_time = end_time - start_time
            print(f"embedding0: {elapsed_time} seconds")
            print(f"embedding_image_feats = {embedding_image_feats.shape}")

            db.fit(features)

            end_time = time.time()
            elapsed_time = end_time - start_time
            print(f"embedding1: {elapsed_time} seconds")
...
class LaneNetPostProcessor(object):
    ...
    """
    def __init__(self, ipm_remap_file_path='./mytraining_data_example/tusimple_ipm_remap.yml'):
        """
    ...
    def postprocess(self, binary_seg_result, instance_seg_result=None,
        ...
        for lane_index, coords in enumerate(lane_coords):
            if data_source == 'tusimple':
                tmp_mask = np.zeros(shape=(960, 544), dtype=np.uint8)
                tmp_mask[tuple((np.int_(coords[:, 1] * 960 / 960), np.int_(coords[:, 0] * 544 / 544)))] = 255
            else:
            ...
            #print(tmp_mask.nonzero()[0].shape)
            #print(tmp_ipm_mask.nonzero()[0].shape)
            ...
            #print("透视变换图片的高度：",ipm_image_height)
            #print("透视变换图片的宽度：",ipm_image_width)
            plot_y = np.linspace(0, ipm_image_height, ipm_image_height - 10)
            ...
            for index in range(0, plot_y.shape[0], 5):
                ...
                lane_pts.append([fit_x[index], plot_y[index]])
        ...
        for index, single_lane_pts in enumerate(src_lane_pts):
            ...
            if data_source == 'tusimple':
                ...
                raise ValueError('Wrong data source now only support tusimple')
            step = int(math.floor((end_plot_y - start_plot_y) / 5))
            singleLane_pts = []
            for plot_y in np.linspace(start_plot_y, end_plot_y, step):
                ...
                if interpolation_src_pt_x > source_image_width or interpolation_src_pt_x < 0:
                    continue

                lane_color = self._color_map[index].tolist()
                singleLane_pts.append([int(interpolation_src_pt_x),int(interpolation_src_pt_y)])
                cv2.circle(source_image, (int(interpolation_src_pt_x),
                                          int(interpolation_src_pt_y)), 5, lane_color, -1)
            mulLane_pts.append(singleLane_pts)
